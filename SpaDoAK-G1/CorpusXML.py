...
#from Include import
from WTimelineA import AnnotatedSequence, keyPremiereTSoffset
...
class CorpusDescriptorDocument(object):
	def __init__(self,filename):
		...
		for se in self.corpusdescr.getElementsByTagName("Session"):
			#print "hallo"
			#print se.attributes['id']
			#print se.getAttribute("id")
...
class XMLCorpusDocument(object):
	...
	def addAnnotatedSequence(self,seqID,aseq,source="?SRC?",video="?VID?", edl="?EDL?"):
		...
		for startTS,startIdx,stopTS,stopIdx,seq in aseq.iterSegments(pointKey = keyPremiereTSoffset):
			...
			### OLD ### textsegment = self.dom.createTextNode(self.text2xml(seq.getLayer('text')))
			
			textdata = seq
			#print "B  TYPE:", type(textdata)
			#print "B  REPR:", repr(textdata)
			#textsegment = self.dom.createTextNode(self.text2xml(textdata).encode('utf-8'))
