...
class LoadedTrajManager(object):
	...
	def loadunloadTrajRessources(self,listOfRessourceIDs):
		...
		newSet = set([])
		#print ("RESOURCES:%s OLDSET:%s" % ( listOfRessourceIDs,oldSet))
		...
		#print "old/new; remove: %s/%s; %s" % (oldSet,newSet,removeSet)
...
class SeqProcessor(object):
	...
	def processAnnotatedSequence(self,aseq,PDFfname,PDFpath,textPageLen,graphPageLen,inIdx=0,outIdx=0,selectedPages=[],outputFileSuffix=None,
		...
		if not simulate:
			...
			#print "    Making new PDF Document."
			#print "A DOCPAGES", pdfDoc, pdfDoc.pages
		...
		#while currstop <= printOutIdx+graphPageLen:
			...
			if spatialPresent:
				...
				if selectedPages == [] or pagecounter in selectedPages:
					...
					if not simulate:
						...
						#print "ALLLOADED:", self.loadedTraj.getAllLoaded()
