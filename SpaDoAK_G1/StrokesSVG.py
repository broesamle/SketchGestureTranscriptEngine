...
#class SVGPathCollection(object)
#	def __init__(self):
#		self.pathsByKey = {}
#
#	def addSVGDocument(self,key,filename):
#		if key in self.pahtsByKey:
#			raise KeyError("Document with Key %s already open." % key)			
#
#		self.pathsByKey[key] = 
#
#
#	def removeSVGDocument(self,key):
...
class SVGDocument(object):
	...
	def handleDocument(self,docel):
		...
		for n in ns:
			...
			if groupmode == 'layer':
				...
				#print n.hasAttributeNS('http://www.inkscape.org/namespaces/inkscape', 'groupmode')
				#print n.getAttribute('inkscape:groupmode')
		
		
	def handleLayer(self,layer):
		#print "layer:", layer.getAttribute('id')
		node = layer.firstChild
		
		while node:
			if node.nodeType == node.ELEMENT_NODE:
				#print "y", node.tagName
				#print "z", node.getAttribute('id')
	...
	def getXYfromSodiPodiNode(self,node):
		...
		### transform="translate(363.96597,164.7226)"
		###            translate(0,2.5390623e-6)
		transf = node.getAttribute('transform') 
		if transf:
			              #translate (    650.80772,-202  .84706 )
			#print transf
