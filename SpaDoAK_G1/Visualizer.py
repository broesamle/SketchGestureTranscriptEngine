...
from WTimelineA import TS_INPOINT, TS_OUTPOINT, BY_INDEX, BY_TIMESTAMP, keyPremiereTSoffset, isNativePremiereTS
...
#	def addSpeaker(self,key,label,colorR,colorG,colorB):
#		if color
#		self.speakers[key] = (label,pyx.color.rgb(colorR,colorG,colorB))
...
#		self.canvas.text(x+margin,y,string3,transf)
	...
	def drawIntervalsBetween(self,aseq,inpointIdx,outpointIdx,svgdocs,textPos=None,transcriptIDprefix="",spatialIDprefix="",drawTrajLabels=False, trajLabelPrefixes=[]):
		...
		#print ("drawIntervalsBetween: %s..%s %d" % (inpointIdx,outpointIdx,self.maxW))
		#maxW = 1300
		...
		if outpointIdx - inpointIdx > self.maxW:
			...
			pos = inpointIdx
			offset = 0
			#print ("split: %d --%d" % (pos,outpointIdx))			
			result = []
			while pos < outpointIdx:
				#print ("recu: %d..%d" % (pos,min(pos+self.maxW,outpointIdx)))
...
#			else:
#				self.drawMarkers()
				#microcanvasesByStopTS[stopTS] = self.canvas 
...
#			except text.TexResultError:
#				raise ValueError( "TEX error: %s (%s..%s)" % (txt,startTS,stopTS))
		...
		for iid,startTS,startIdx,stopTS,stopIdx,intervaldata,subseq in aseq:
			...
			if intervaldata['IntervalType'] == 'STROKE':
				...
				#print transcriptIDprefix,":PRE:",spatialIDprefix
				#print spatElID
				...
				#print "XX:%s YY:%s" % (self.lastPrintedStrokeID,spatElID )
				...
				try:
					...
					for svgdoc in svgdocs:
						try:
							...
							#raise Aerr
						if svgpathdata:
							### we got something -- don't go on looping
							break
	...
	def drawStroke(self,traj,intervaldata,decoration):
		...
		if 'holdstart' in intervaldata:
			holdstart = intervaldata['holdstart']
		else:
			holdstart = ""
		
		if 'holdstop' in intervaldata:
			holdstop = intervaldata['holdstop']
		else:
			holdstop = ""

		if 'hold' in intervaldata:
			hold = intervaldata['hold']
		else:
			hold = ""
